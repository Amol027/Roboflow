...
from app.models import classification_training

logger = logging.getLogger(__name__)

class MobileNetTrainer:
    def __init__(self, project_id, num_classes, class_names):
    ...
    def process_and_train(self, zip_bytes):
        ...
        history = self.model.fit(train_gen, validation_data=val_gen, epochs=5, callbacks=[callback])
    ...
    def load_local(model_id):
        ...
        with tempfile.NamedTemporaryFile(suffix=".h5", delete=False) as tmp:
            ...
            tmp.flush()
            trainer.model = load_model(tmp.name)
        os.unlink(tmp.name)
