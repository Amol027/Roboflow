...
class DatasetSplitterAPI:
    ...
    def try_load_model(self):
        try:
            if not os.path.exists(self.model_path):
                ...
                url = f"https://github.com/ultralytics/assets/releases/download/v0.0.0/{self.model_path}"
                urllib.request.urlretrieve(url, self.model_path)
                logger.success(f"{self.model_path} downloaded.")
    ...
    def register_routes(self):
        ...
        async def root():
            return {
                "message": "YOLOv8n Dataset Splitter API",
                "usage": "/split (POST)",
...
if __name__ == "__main__":
    uvicorn.run("yolov8n_app:app", host="127.0.0.1", port=8000, reload=True)
