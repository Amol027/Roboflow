...
app = FastAPI(title="MobileNet Model Trainer")
...
@app.get("/")
def read_root():
    return {"message": "Welcome to the MobileNet Model Trainer API"}
...
@app.post("/train")
async def train_mobilenet(file: UploadFile = File(...)):
    return handle_upload_and_train(file, "mobilenet")
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("mobilenet:app", host="127.0.0.1", port=8000, reload=True)